...
// Special constant that makes the app run on the NV GPU on Optimus laptops
extern "C"
{
...
HRESULT DeviceManager::CreatePrePostRenderCommandLists()
{
    ...
    for (UINT bufferIndex = 0; bufferIndex < m_SwapChainDesc.BufferCount; bufferIndex++)
    {
        ID3D12GraphicsCommandList* pPreRender = nullptr;
        hr = m_Device12->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_DefaultCommandAllocator, NULL, IID_PPV_ARGS(&pPreRender));
        HR_RETURN(hr);

        SetResourceBarrier(pPreRender, m_SwapChainBuffers[bufferIndex], D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
        if (m_EnableRenderTargetClear)
        {
            pPreRender->ClearRenderTargetView(m_SwapChainRTVs[bufferIndex], m_RenderTargetClearColor, 0, NULL);
        }
        pPreRender->Close();

        m_PreRenderCommandLists.push_back(pPreRender);


        ID3D12GraphicsCommandList* pPostRender = nullptr;
        hr = m_Device12->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_DefaultCommandAllocator, NULL, IID_PPV_ARGS(&pPostRender));
        HR_RETURN(hr);

        SetResourceBarrier(pPostRender, m_SwapChainBuffers[bufferIndex], D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
        pPostRender->Close();

        m_PostRenderCommandLists.push_back(pPostRender);
    }
...
DeviceManager::OneFrame(double fElapsedTimeSeconds)
{
    ...
    if(SUCCEEDED(m_SwapChain->GetDesc1(&newSwapChainDesc)) && SUCCEEDED(m_SwapChain->GetFullscreenDesc(&newFullScreenDesc)))
    {
        ...
        {
            ReleaseRenderTargets();
            newSwapChainDesc.Width = m_NewWindowSize.cx;
            newSwapChainDesc.Height = m_NewWindowSize.cy;
    ...
    m_InsidePresent = true;
    m_SwapChain->Present(m_SyncInterval, 0);
    m_InsidePresent = false;
...
{
    m_DefaultQueue->ExecuteCommandLists(1, (ID3D12CommandList**)&m_PreRenderCommandLists[m_BufferIndex]);
    ...
    m_DefaultQueue->ExecuteCommandLists(1, (ID3D12CommandList**)&m_PostRenderCommandLists[m_BufferIndex]);
}
    ...
    switch(uMsg)
    {
        ...
        case WM_SIZE:
            ...
            if (m_Device12 && (lParam != 0))
            {
                ...
                if(!m_InSizingModalLoop && !m_InsidePresent)
                    ResizeSwapChain(true);
            }
    }
...
void DeviceManager::ReleaseRenderTargets()
{
    ...
    for (auto pCL : m_PreRenderCommandLists)
        pCL->Release();
    m_PreRenderCommandLists.clear();

    for (auto pCL : m_PostRenderCommandLists)
        pCL->Release();
    m_PostRenderCommandLists.clear();
}
